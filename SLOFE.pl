...
#	system('paste '.$tmp_dir.'/final_stem-loops_improved '.$tmp_dir.'/Poly_T | grep -v \').*(\' > final_stem-loops_improved_T');
	system('paste '.$tmp_dir.'/final_stem-loops_improved '.$tmp_dir.'/Poly_T | grep -v \').*(\' > '.$dirname_abs.'/stable_stem-loops');
	my $stable_stemloops= `wc -l $dirname_abs/stable_stem-loops`;
	print "Number of stable stem-loops: $stable_stemloops";
	system('grep insidOP '.$dirname_abs.'/stable_stem-loops |grep -v inside > '.$dirname_abs.'/intergenic_intraOperonic_stem-loops');
	print "Stable stem-loops prediction finished\n";
	print "Ratio Calculation.......\n";	
	system('perl '.$dirname_abs.'/ratio_calc.pl  '.$Operon.' '.$GB.'');
}
