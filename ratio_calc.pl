...
system_commands($Operon, $GB, $dir_out);
...
open (RatioCalc, ">$dir_out/Stoichiometry_ratio.csv");
my $b=0; my $wc=0;
...
print "Number of SRPS operons and their stoichiometry in the from of ratio: $wc $dir_out/Stoichiometry_ratio.csv\n";
print "Done!\n";

#system('mv '.$tmp_dir.'/rnamotif.out '.$tmp_dir.'/sequence.out '.$tmp_dir.'/rnafold.out '.$tmp_dir.'/cutoff5_stem-loops '.$tmp_dir.'/non-redundant_stem-loops '.$tmp_dir.'/final_stem-loops_sorted '.$tmp_dir.'/T_ext '.$dir_out.'');
#system('rm -rf '.$tmp_dir.'/*');
#system('mv ../rnamotif.out ../rnafold.out ../rnaM_rnaF_merged ../final_stem-loops_improved_T_intergenic ../with_eng_redef '.$tmp_dir.'/');
#system('mv '.$dir_out.'/rnamotif.out '.$dir_out.'/sequence.out '.$dir_out.'/rnafold.out '.$dir_out.'/cutoff5_stem-loops '.$dir_out.'/non-redundant_stem-loops '.$dir_out.'/final_stem-loops_sorted '.$dir_out.'/T_ext  '.$tmp_dir.'/');
...
sub system_commands{
	...
	system('grep -v Poly5U '.$dir_out.'/intergenic_intraOperonic_stem-loops |grep -v Poly4U|cut -f17|sort -g -u > '.$tmp_dir.'/tmp_srps_uniqOP');
	#Stabilizer_stem-loops-SSL
        system('grep -v Poly5U '.$dir_out.'/intergenic_intraOperonic_stem-loops |grep -v Poly4U|awk \'{FS="\t"}{if($19<=5)print $0}\' > '.$dir_out.'/Stabilizer_stem-loops-SSL');
	$SSL= `wc -l $dir_out/Stabilizer_stem-loops-SSL`;
	print "Number of predicted SSL: $SSL"; 
	#Stabilizer_and_terminator_stem-loops-STSL
        system('grep -v Poly5U '.$dir_out.'/intergenic_intraOperonic_stem-loops |grep -v Poly4U|awk \'{FS="\t"}{if($19>5)print $0}\' > '.$dir_out.'/Stabilizer_and_terminator_stem-loops-STSL');
	$STSL= `wc -l $dir_out/Stabilizer_and_terminator_stem-loops-STSL`;
	...
	system('awk \'{FS="\t"}NR==FNR{h[$0]=1; next}{for(i in h) if($0~i){print $0"\t"i}}\' '.$tmp_dir.'/tmp_all_srpsGene '.$tmp_dir.'/improved_operon_map1|cut -f1,3,6,7 > '.$tmp_dir.'/genes_with_operon_number_new');
	...
	system('grep -v inside '.$dir_out.'/stable_stem-loops > '.$tmp_dir.'/final_stem-loops_improved_T_intergenic');
}
