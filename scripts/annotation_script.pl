...
Usage: ./gene_annotation <annotation_file_in_genbank_format> \n Output file name will be gene_annotation_<file-name> \n";}
...
sub system_commands{
	#system('mkdir tmp');
	system('grep -A2 "gene    " ' .$op.'|grep -v "/db_xref\|/gene\|/pseudogene\|/old_locus_tag\|/pseudo\|CDS\|ncRNA"|sed -e \'/--/d\' -e \'s/ //g\' -e \'/^\/locus/s/$/@/\'|tr -d \'\n\'|tr \'@\' \'\n\'|sed -e \'s/gene//\' -e \'s/\.\./\t/\' -e \'s/\/locus_tag="/\t/\' -e \'s/"//\'|awk \'{FS="\t"}{if($1~/complement/) {print $1"\t"$2"\t"$3"\t-"} else {print $1"\t"$2"\t"$3"\t+"}}\'|sed -e \'s/complement(//\' -e \'s/)//\' -e \'s/>//\' -e \'s/<//\' > gene_annotation_'.$op.'')
	}
